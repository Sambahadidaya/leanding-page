...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!sessionStorage.getItem('visitTracked')) {
    try {
      ...
      console.log('Kunjungan tercatat.');
    } catch (err) {
...
function renderActiveCandidate(candidate) {
  ...
  if (candidateName) {
    candidateName.textContent = candidate.nama;  // Ubah candidate.name ke candidate.nama
    ...
    if (error) {
      ...
      const { error: updateError } = await supabaseClient
        ...
        .update({
          ...
          pilihan: calon,  // Nama kandidat yang dipilih
          pilihanID: calon_id,  // ID kandidat (nomor kandidat)
          waktu_memilih: waktu  // Waktu voting
        })
        .eq('NIM', nim);
      if (updateError) {
...
async function confirmVote(candidateId, candidateName, candidateNo) {  // PERUBAHAN: Tambahkan candidateNo
  setLoadingState('confirmVoteBtn', true);
  closeModal();
  const selectedCandidateElement = document.getElementById("selectedCandidate");
  if (selectedCandidateElement) selectedCandidateElement.textContent = candidateName;
  ...
  await sendVoteToSupabase(
    ...
    voter.password,
    candidateName,
    candidateNo,  // PERUBAHAN: Gunakan candidateNo (No kandidat) sebagai calon_id
    voter.Angkatan,
    voter.Prodi,
  );
  hasVoted = true;
...
// INISIALISASI
// ====================
// Hapus atau komentari updateResults() di sini jika tidak perlu di awal, agar tidak error sebelum login
// updateResults();  // Komentari ini
showPage("loginForm");